...
"""
A file with the key logic of the bot, namely the collection 
of information for the form from the user. 
All handlers are connected by a state machine, which is declared 
in the f-ai, called by the /form command (in the echo file)
"""
...
async def form_check_results(callback: types.CallbackQuery, state: FSMContext):
    ...
    if key == 'ok':
        ...
        await callback.message.edit_text('Данные успешно отправлены 🤝\n'
                                         'После заполнения формы, мы уведомим вас (не более 10 мин)')
